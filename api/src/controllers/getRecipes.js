//CONTROLLERS FUNCIONES PARA TRAER LOS DATOS
    ...
    const apiUrl = await axios.get(`https://api.spoonacular.com/recipes/complexSearch?apiKey=${API_KEY}&number=100&addRecipeInformation=true`)
     ...
     const apiInfo = await apiUrl.data.results.map(e =>{
         return {
             ...
             dishTypes: e.dishTypes.map((ele) =>{return{ele}}),
...
async function getAallRecipes(req, res) {
    ...
    if (!name) {                                        // si no viene ningun nombre entra al if
      try {
        ...
        return res.send(await Promise.all([...recipeBD,...recipeApiInfo,])); // una vez que terminan todas la promesas, le pido que concatene todas la recetas
